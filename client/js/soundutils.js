async function loadAndDecodeSound(url, ctx) {
  ...
  // start now, play the segment [startTime, endTime]
  const now = ctx.currentTime;
  bufferSource.start(0, startTime, endTime);

  // compute play duration in seconds
  const playDuration = Math.max(0, endTime - startTime);

  // return useful metadata so callers can sync UI (playhead)
  return { bufferSource, startedAt: now, playDuration };
}
